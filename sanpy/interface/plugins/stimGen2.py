    ...
    for param in commentStr.split(";"):
        ...
        if v in ["True", "False"]:
            ...
            try:
                ...
                logger.error(
                    f'ERROR: ValueError for key:{k} value:"{v}" param="{param}"'
                )
        #
        retDict[k] = v
    #
...
class myDoubleSpinBox(QtWidgets.QDoubleSpinBox):
    def __init__(
        self, statName, value, minVal=0, maxVal=1e9, decimals=3, callback=None
    ):
        """
...
class stimGen(sanpyPlugin):
    ...
    def makeStim2(self):
        ...
        for sweepNum in range(numStimSweeps):
            ...
            print(
                f"  makeStim() {stimType} sweep:{sweepNum} durSec:{stimDur_sec} amp:{currAmp} freq:{currFreq} noiseAmp:{currNoiseAmp}"
            )
            self._data[realSweepNum] = sanpy.atfStim.makeStim(
                stimType,
                sweepDurSec=sweepDur_sec,
                startStimSec=stimStart_sec,
                durSec=stimDur_sec,
                yStimOffset=yOffset,
                amp=currAmp,
                freq=currFreq,
            ...
            print(
                f"  {realSweepNum} self._data[realSweepNum].dtype:{self._data[realSweepNum].dtype}"
            )
