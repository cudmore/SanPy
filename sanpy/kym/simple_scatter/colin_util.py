...
# abb 20250617
def _parseFijiRoiName(fijiRoiName:str,
                      dateFolder:str,
                      regionFolder:str,
                      fijiRoiRect:list,
                      sanpyRoiRect:list,
    ...
    tifFileName = f'{dateFolder} {regionFolder} {roiName2} {sanpyCond}'
    if retDict["Epoch"] > 0:
...
def loadRoiFile(masterDict, oneZipFile) -> pd.DataFrame:
    ...
    for fijiRoiNumber, roi in enumerate(fijiRoiList):
        ...
        roiInfoDict = _parseFijiRoiName(fijiRoiName, _dateFolder, _regionFolder, fijiRoiRect, sanpyRect)
...
def makeRoiDictFromZips() -> tuple[pd.DataFrame, dict, list]:
    ...
    zipList2 = ['20250602/ISAN/ISAN RoiSet.zip',
...
def _setDetection_df_f0(kymRoiAnalysis:KymRoiAnalysis,
                         roiLabel : str,
                         cond : str):
...
def _setDetection_Divide(kymRoiAnalysis:KymRoiAnalysis,
                         roiLabel : str,
                         cond : str):
...
def insertRoiIntoSanPy():
    ...
    for tifIdx, rawTifPath in enumerate(rawTifList):
        ...
        # print(dfTif)
        if len(dfTif) == 0:
            logger.error(f'did not find any rois for tif file "{tifFile}" in roi dfMaster')
            logger.error('-->> exit')
            sys.exit(1)
