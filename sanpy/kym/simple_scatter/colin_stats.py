...
def makeMeanDf():
    ...
    for index, row in dfGrouped.iterrows():
        ...
        detectionParams = ka.getDetectionParams(
            roiNumber, PeakDetectionTypes.intensity, channel=0
        )
    ...
    dfGrouped['Epoch'] = dfGrouped['Tif File'].apply(
        lambda x: FileInfo.from_path(x).epoch
    )
    ...
    dfGrouped['Condition Epoch'] = (
        dfGrouped['Condition'].fillna('')
        + ' '
        + dfGrouped['Epoch'].astype(str).fillna('')
    ).str.strip()
    dfGrouped['Condition Epoch'] = dfGrouped['Condition Epoch'].astype('category')
...
def _removeOneSpikeControl(dfMean: pd.DataFrame):
    ...
    logger.info(
        f'num Cell Id Control with <= {removeLessThanEqual} peaks is {numControlWithOneSpike}'
    )
    ...
    for (
        rowLabel,
        rowDict,
    ) in dfOneSpike.iterrows():  # iterate our <=1 spike (cell id, roi number)
