...
from sanpy.kym.simple_scatter.colin_global import (
    loadMasterDfFile,
    loadMeanDfFile,
    getCellIdPdfFolder,
...
def plotRois():
    ...
    for cellID in cellIDs:
        ...
        for row in iterate_unique_cell_rows(dfMean, cellID):
            ...
            # save the figure
            pdfPath = getCellIdPdfFolder(
                pdfOutputType='Per Cell ROI Plots',
                cellId=cellID,
                dateStr=dateStr,
                regionStr=regionStr,
            )
...
def _exportClips(cellID):
    ...
    pdfPath = getCellIdPdfFolder(
        pdfOutputType='Per Cell Cond Plots',
        cellId=cellID,
        dateStr=dateStr,
        regionStr=regionStr,
    )
...
def new_plotCellID(
    dfMaster, dfMean, cellID, roiLabelStr: int
) -> tuple[plt.Figure, plt.Axes, dict]:
    roiLabelInt = int(roiLabelStr)
    channel = 0

    # get rows across all Epoch, each row becomes a column
    theseRows = (dfMean['Cell ID'] == cellID) & (dfMean['ROI Number'] == roiLabelInt)
    ...
    for rowIdx, (rowLabel, row) in enumerate(dfPlot.iterrows()):
        ...
        plotStatSwarm(
            dfMean, 'Number of Spikes', cellID, epoch, roiLabelStr, ax[plotRow][0]
        )

        # make 2x swarm plot (height, mass)
        plotStatSwarm(
            dfMaster, 'Peak Height', cellID, epoch, roiLabelStr, ax[plotRow][1]
        )
        plotStatSwarm(
            dfMaster, 'Area Under Peak', cellID, epoch, roiLabelStr, ax[plotRow][2]
        )
...
def _plotQuality():
    ...
    for cellID in cellIDs:
        ...
        for roiLabelStr in roiLabels:
            ...
            pdfPath = getCellIdPdfFolder(
                pdfOutputType='ROI Quality Plots',
                cellId=cellID,
                dateStr=dateStr,
                regionStr=regionStr,
            )
...
def plotCellID():
    ...
    for cellID in cellIDs:
        ...
        for roiLabelStr in roiLabels:
            ...
            fig, ax, imgDataDict = new_plotCellID(
                dfMaster, dfMean, cellID, roiLabelStr=roiLabelStr
            )
