...
def _printImgStats(imgData: np.ndarray, name: str = ''):
    logger.info(
        f'name:"{name}" imgStats: {imgData.shape} min:{np.min(imgData)} max:{np.max(imgData)}'
    )

def findThreshold(
    kymRoiTrace: np.ndarray,
    polarity: str,
    ...
    for _idx, peakBin in enumerate(peakBins):
        ...
        if polarity == 'Pos':
            threshold = yPeak - (
                peakHeight * newOnsetOffsetFraction
            )  # 0.9 is 90% height
        elif polarity == 'Neg':
            threshold = yPeak + (
                peakHeight * newOnsetOffsetFraction
        ...
        if polarity == 'Pos':
            threshold_crossings = np.diff(
                int_df_f0 > threshold, append=False
            )  # "False" is the value to append
        elif polarity == 'Neg':
            threshold_crossings = np.diff(
                int_df_f0 < threshold, append=False
            )  # "False" is the value to append
        ...
        if polarity == 'Pos':
            threshold_crossings = np.diff(
                int_df_f0 > threshold, prepend=False
            )  # "False" is the value to append
        elif polarity == 'Neg':
            threshold_crossings = np.diff(
                int_df_f0 < threshold, prepend=False
        ...
        if len(decayBins) == 0:
            ...
            if polarity == 'Pos':
                threshold_crossings3 = np.diff(
                    int_df_f0 > threshold3, append=False
                )  # "False" is the value to append
            elif polarity == 'Neg':
                threshold_crossings3 = np.diff(
                    int_df_f0 < threshold3, append=False
        ...
        if doMpl and _idx in [1, 2]:
            ...
            # peak
            axs[0].plot(
                peakBin, int_df_f0[peakBin], color='c', marker='^', label='peak'
            )
...
class KymRoiTraces:
    ...
    def setTrace2(self, traceName, bins: np.ndarray, values: np.ndarray):
        ...
        if traceName not in self._analysisTraces.keys():
            logger.error(
                f'traceName "{traceName}" not in keys, available keys are {self._analysisTraces.keys()}'
            )
            return
    ...
    def getTrace(
        ...
        if name not in self._analysisTraces.keys():
            logger.error(
                f'did not find trace name "{name}". Available names are {self._analysisTraces.keys()}'
            )
            return
    ...
    def __getitem__(self, key):
        ...
        try:
            ...
            logger.error(
                f'Error getting trace key "{key}" available keys are {self._analysisTraces.keys()}'
            )
            raise
        #
        return ret
...
class KymRoi:
    ...
    def __init__(
        ...
        """
        self._label: str = label # like ROI 1, ROI 2, ROI 3, ...
        ...
        from sanpy.kym.kymRoiAnalysis import KymRoiAnalysis  # circular imports
        self._kymRoiAnalysis: KymRoiAnalysis = kymRoiAnalysis

        if skip_constraint:
            # When loading from analysis files, coordinates are already constrained
            self._ltrbRect = ltrbRect
        else:
            # When creating new ROIs, constrain the coordinates
            self.setRect(ltrbRect)  # will constrain

        # self._isDirty = False
        self.setDirty(False)

        # Get number of channels from header if available, otherwise from imgData
        if header and 'numChannels' in header:
            _numChannels = header['numChannels']
        elif imgData is not None:
            _numChannels = len(imgData)
        else:
            # Default to 1 channel if we can't determine
            _numChannels = 1
        ...
        for channel in range(_numChannels):
            ...
            for peakDetectionType in PeakDetectionTypes:
                if reuseRoiLabel is not None:
                    _reuseKymRoiDetection = kymRoiAnalysis.getDetectionParams(
                        reuseRoiLabel, peakDetectionType, channel
                    )
                    thisDetection = KymRoiDetection(
                        peakDetectionType, kymRoiDetection=_reuseKymRoiDetection
                    )
                else:
            ...
            # Initialize traces with number of line scans from header if available
            if header and 'imageWidth' in header:
                numLineScans = header['imageWidth']
            elif imgData is not None and channel < len(imgData):
    ...
    def getAnalysisResults(
        self, channel: int, detectionType: PeakDetectionTypes
    ) -> KymRoiResults:
    ...
    def setResults(
        self, channel, detectionType: PeakDetectionTypes, kymRoiResults: KymRoiResults
    ) -> KymRoiResults:
    ...
    def setUserLabel(self, userLabel: str):
    ...
    def path(self):
        ...
        return self._header.getParam('path')

    @property
    def umPerPixel(self):
        return self._header.getParam('umPerPixel')

    @property
    def secondsPerLine(self):
        return self._header.getParam('secondsPerLine')

    def getDefaultRect(self):
        # Try to get dimensions from header first, then from image data
        if self.header and self.header.getParam('imageHeight') is not None and self.header.getParam('imageWidth') is not None:
            left = 0
            top = self.header.getParam('imageHeight')
            right = self.header.getParam('imageWidth')
            bottom = 0
        elif self._imgData is not None:
            # h, w = self._imgData.shape
            left = 0
            top = self._imgData[0].shape[0]  # height
            right = self._imgData[0].shape[1]  # width
            bottom = 0
        else:
            # Default to reasonable dimensions if we can't determine
            left = 0
            top = 1000  # default height
            right = 1000  # default width
            bottom = 0
    ...
    def getRectDict(self) -> dict:
        ...
        ltrb = self.getRect()
        return {'left': ltrb[0], 'top': ltrb[1], 'right': ltrb[2], 'bottom': ltrb[3]}
    ...
    def nudge(self, direction: str):
        ...
        logger.info(
            f'   roi nudge "{direction}" delta:{delta} orig ltrb:{self.getRect()}'
        )
        self.setRect(ltrb)
    ...
    def _getConstrainedRoi(self, ltrb) -> List[int]:
        ...
        # Get dimensions from header first, then from image data
        if self.header and self.header.getParam('imageHeight') is not None and self.header.getParam('imageWidth') is not None:
            heightMax = self.header.getParam('imageHeight')
            widthMax = self.header.getParam('imageWidth')
        elif self._imgData is not None:
            heightMax = self._imgData[0].shape[0]
            widthMax = self._imgData[0].shape[1]
        else:
            # Default to reasonable dimensions if we can't determine
            heightMax = 1000
            widthMax = 1000
    ...
    def getRoiImg(self, channel) -> np.ndarray:
        ...
        """
        # Use parent's image data if ROI doesn't have its own (e.g., in analysis_only mode)
        if self._imgData is None:
            if self._kymRoiAnalysis is None or self._kymRoiAnalysis._imgData is None:
                raise RuntimeError(
                    f"Image data not available for ROI {self._label}. "
                    "This can happen in analysis_only mode when trying to access image data."
                )
            imgData = self._kymRoiAnalysis._imgData
        else:
            imgData = self._imgData
            
        left, top, right, bottom = self.getRect()
        roiImg = imgData[channel][bottom:top, left:right]
        roiImg = np.copy(roiImg)
        return roiImg

    def backgroundSubtract(
        self, channel, roiImg, peakDetectionTypes: PeakDetectionTypes
    ):
        ...
        if backgroundsubtract == 'Off':
            ...
            rollingBackground = restoration.rolling_ball(
                roiImg, radius=_rollingBallRadius
            )
            ...
            logger.error(
                f'did not understand background subtract "{backgroundsubtract}" -->> no subtraction'
            )
    ...
    def getRoiImgClips(self, channel) -> Dict:
        ...
        # "background" subtract
        roiImg_bs = self.backgroundSubtract(
            channel, roiImg_raw, PeakDetectionTypes.intensity
        )
        ...
        santanaLineScanNorm = self._kymRoiAnalysis.getKymDetectionParam(
            'Divide Line Scan'
        )
        ...
        if santanaLineScanNorm is None:
            ...
            try:
                ...
                roiImg_divided = np.divide(
                    roiImgFloat,
                    reshaped_column_float,
                    out=ones_like,
                    where=reshaped_column != 0,
                )

                # this col should be all 1
                if not np.all(roiImg_divided[:, santanaLineScanNorm] == 1):
                    logger.error(
                        f'santanaLineScanNorm:{santanaLineScanNorm} column should be all 1'
                    )
    ...
    def getSumIntensity(self, channel):
        ...
        if detectionParams['Median Filter']:
            ...
            logger.info(
                f'   applying median filter with medianfilterkernel:{medianfilterkernel}'
            )
        ...
        if postMedianFilterKernel > 0:
            ...
            if postMedianFilterKernel % 2 == 0:
                logger.warning(
                    f'forcing odd postMedianFilterKernel:{postMedianFilterKernel}'
                )
    ...
    def getLineProfile(self, channel, lineIdx):
        ...
        line_interp_mult_diam = detectionParams[
    ...
    def getPeakClips(
        ...
        channel: int,
        asPercent: bool = True,
        plusMinusMs: float = 50,
    ) -> tuple[np.ndarray, np.ndarray]:

        xPeakBins = self.getAnalysisResults(channel, peakDetectionType).getValues(
            'Peak Bin'
        )
        numPeaks = len(xPeakBins)
        ...
        detectThisTrace = self.getDetectionParams(channel, peakDetectionType).getParam(
            'detectThisTrace'
        )
        ...
        xOneClip = [
            (x - plusMinusBins) * self.secondsPerLine for x in range(numPntsInClip)
        ]
        ...
        for peakIdx, xPeak in enumerate(xPeakBins):
            ...
            try:
                ...
                logger.warning(
                    f'   (1) not calculating clip for peak: {peakIdx} --> {e}'
                )
            ...
            try:
                ...
                logger.warning(
                    f'   (2) not calculating clip for peak: {peakIdx} --> {e}'
                )
    ...
    def detectDiam(self, channel, verbose: bool = False):
        ...
        logger.info(
            f'==>> calling detectKymRoiDiam() with roi {self.getLabel()} {roiImg.shape}'
        )
        ...
        leftThresholdBins, rightThresholdBins, diameterBins, sumIntensity = (
            detectKymRoiDiam(
                roiImg,  # roiImg is clipped to ltrb of roi rect
                doBackgroundSubtract=doBackgroundSubtract,
                lineWidth=lineWidth,
        ...
        self.setTrace2(
            ...
            'Left Diameter (um)',
            _timeBins,
            leftThresholdBins * self.umPerPixel,
        )
        self.setTrace2(
            channel,
            'Right Diameter (um)',
            _timeBins,
            rightThresholdBins * self.umPerPixel,
        )
        self.setTrace2(
            channel, 'Diameter (um)', _timeBins, diameterBins * self.umPerPixel
        )
    ...
    def peakDetect(
        ...
        detectionParams = self.getDetectionParams(
            channel, detectionType=peakDetectionType
        )
        ...
        detectThisTrace = detectionParams[
            'detectThisTrace'
        ]  # from (df_f0, f_f0, Diameter (um))
        ...
        if peakDetectionType == PeakDetectionTypes.diameter:
            ...
            if np.isnan(_tmpTrace).all():
                logger.error(
                    f'trace "{detectThisTrace}" has no value, did you perform "detect diameter"?-->> no detection performed'
                )
                return
        ...
        if _do__manual_f0 and detectionParams['f0 Value Manual'] is None:
            logger.error(
                'ManualPercentile is set to "Manual" but "f0 Value Manual" is None --> user needs to set value'
            )
            return
        if _do__manual_f0:
        ...
        if peakDetectionType == PeakDetectionTypes.diameter:
            ...
            xPlot, yRaw, dividedInt = self.getSumIntensity(
                channel
            ...
            if verbose:
                logger.info(
                    f'   -->> using f0 Type: "{f0ManualPercentile}" f0Percentile:{f0Percentile} f0 is :{f0}'
                )
        ...
        yPeaks = yDf_f0[
            peakBins
        ]  # yDf_f0 is NOT inverted on 'Neg' polarity (always raw trace to analyze)
        ...
        #
        thresh_rel_height = detectionParams[
            'thresh_rel_height'
        ]  # using default of 0.85, 1 is foot, 0 is peak
        ...
        _peakInterval = np.insert(
            _peakInterval, 0, np.nan
        )  # insert np.nan as first (0) element
        ...
        hwSeconds = ((peakDict['right_ips'] + _left) * self.secondsPerLine) - (
            (peakDict['left_ips'] + _left) * self.secondsPerLine
        )
        hwMs = hwSeconds * 1000
        ...
        newOnsetSeconds = [
            (_bin + _left) * self.secondsPerLine for _bin in newOnsetBins
        ]
        newDecaySeconds = [
            (_bin + _left) * self.secondsPerLine for _bin in newDecayBins
        ]

        newOnset10Seconds = [
            (_bin + _left) * self.secondsPerLine for _bin in newOnset10Bins
        ]
        newDecay10Seconds = [
            (_bin + _left) * self.secondsPerLine for _bin in newDecay10Bins
        ]
        ...
        _tmp_yDf_f0 = [
            yDf_f0[_newOnsetBin] if ~np.isnan(_newOnsetBin) else np.nan
            for _newOnsetBin in newOnsetBins
        ]
        # oneRoiResults.setValues('Onset Int', yDf_f0[newOnsetBins])  # yPlot is already reduced to roi (left)
        oneRoiResults.setValues(
            'Onset Int', _tmp_yDf_f0
        )  # yPlot is already reduced to roi (left)
        # peak height is wrt onset (not decay)
        peakHeight = np.subtract(
            oneRoiResults.getValues('Peak Int'), oneRoiResults.getValues('Onset Int')
        )
        oneRoiResults.setValues('Peak Height', peakHeight)
        ...
        _tmp_yDf_f0 = [
            yDf_f0[_newOnset10Bin] if ~np.isnan(_newOnset10Bin) else np.nan
            for _newOnset10Bin in newOnset10Bins
        ]
        # oneRoiResults.setValues('Onset 10 Int', yDf_f0[newOnset10Bins])  # yPlot is already reduced to roi (left)
        oneRoiResults.setValues(
            'Onset 10 Int', _tmp_yDf_f0
        )  # yPlot is already reduced to roi (left)
        ...
        _tmp_yDf_f0 = [
            yDf_f0[_newDecayBin] if ~np.isnan(_newDecayBin) else np.nan
            for _newDecayBin in newDecayBins
        ]
        # oneRoiResults.setValues('Decay Int', yDf_f0[newDecayBins]) # yPlot is already reduced to roi (left)
        oneRoiResults.setValues(
            'Decay Int', _tmp_yDf_f0
        )  # yPlot is already reduced to roi (left)
        ...
        _tmp_yDf_f0 = [
            yDf_f0[_newDecay10Bin] if ~np.isnan(_newDecay10Bin) else np.nan
            for _newDecay10Bin in newDecay10Bins
        ]
        # oneRoiResults.setValues('Decay 10 Int', yDf_f0[newDecay10Bins]) # yPlot is already reduced to roi (left)
        oneRoiResults.setValues(
            'Decay 10 Int', _tmp_yDf_f0
        )  # yPlot is already reduced to roi (left)
        ...
        oneRoiResults.setValues(
            'Onset 90 (s)', (peak90Bin + _left) * self.secondsPerLine
        )
        oneRoiResults.setValues(
            'Onset 90 Int', yDf_f0[peak90Bin]
        )  # yPlot is already reduced to roi (left)
        ...
        oneRoiResults.setValues(
            'Decay 90 (s)', (decay90Bin + _left) * self.secondsPerLine
        )
        oneRoiResults.setValues(
            'Decay 90 Int', yDf_f0[decay90Bin]
        )  # yPlot is already reduced to roi (left)

        #
        # colin symposium, full width from onset to offset (using peak "10" detection)
        fwSeconds = ((peak10_right_ips + _left) * self.secondsPerLine) - (
            (peak10_left_ips + _left) * self.secondsPerLine
        )
        fwMs = fwSeconds * 1000
        oneRoiResults.setValues('FW (ms)', fwMs)

        # colin, full with from rise 10 to decay 10
        fw10Seconds = oneRoiResults.getValues('Decay 10 (s)') - oneRoiResults.getValues(
            'Onset 10 (s)'
        )
        ...
        decay1090Seconds = oneRoiResults.getValues(
            'Decay 10 (s)'
        ) - oneRoiResults.getValues(
            'Decay 90 (s)'
        )  # element wise subtract
        ...
        fit_m, fit_tau, fit_b, fit_r2, fit_error = _expFit(
            xPlot, yDf_f0, peakBins, decayFitBins=decayFitBins
        )
        ...
        fit_m1, fit_tau1, fit_m2, fit_tau2, fit_r22, fit_error = _expFit2(
            xPlot, yDf_f0, peakBins, decayFitBins=decayFitBins
        )
...
def getAreaUnderPeak(yPlot, onsetBin, offsetBin, baseline: float = None) -> float:
    ...
    try:
        ...
        logger.error(e)
        logger.error(
            f'colin 20250521 onsetBin:{onsetBin} offsetBin:{offsetBin} onsetBaseline:{onsetBaseline}'
        )
        return np.nan
