...
class KymRoiGroupBox(QtWidgets.QGroupBox):
    ...
    def slot_selectRoi(self, channel : int, roiLabel : Optional[str]):
        ...
        if roiLabel is not None:
            ...
            logger.info(f'  roi _rectDict:{_rectDict} ')  # pixels like [0, 512, 2000, 0]
    ...
    def _on_spin_box(self, name, value):
        ...
        if name == 'Height':
            ...
            self._kymRoiAnalysis.getRoi(roiLabel=self._roiLabel).setRoiHeightPixels(value)
    ...
    def _buildUI(self):
        ...
        _heightSpinBox.valueChanged.connect(
        ...
        _nudgeLeftButton.clicked.connect(
            partial(self.nudgeRoi, direction='left')
            )
        _nudgeRightButton.clicked.connect(
            partial(self.nudgeRoi, direction='right')
            )
        _nudgeUpButton.clicked.connect(
            partial(self.nudgeRoi, direction='up')
            )
        _nudgeDownButton.clicked.connect(
            partial(self.nudgeRoi, direction='down')
            )
