...
from PyQt5.QtWidgets import (QApplication, QWidget, QVBoxLayout, QHBoxLayout, 
                             QToolBar, QPushButton, QCheckBox, QSplitter, 
                             QLabel, QMainWindow, QAction, QMenu)
...
class CustomWidget(QWidget):
    ...
    def init_ui(self):
        ...
        # Create splitter for the three widgets
        self.splitter = QSplitter(Qt.Vertical)
        
        # Create three widgets with different background colors and store in dictionary
        self.widgets = {
            "Widget 1": self.create_colored_widget("Widget 1", "#FFE6E6"),  # Light red
            "Widget 2": self.create_colored_widget("Widget 2", "#E6F3FF"),  # Light blue  
            "Widget 3": self.create_colored_widget("Widget 3", "#E6FFE6")   # Light green
        }
    ...
    def create_toolbar(self):
        ...
        self.toolbar_widgets = {
            "Button 1": QPushButton("Button 1"),
            "Button 2": QPushButton("Button 2"),
            "Option 1": QCheckBox("Option 1"),
            "Option 2": QCheckBox("Option 2"),
            "Text Input": QLineEdit(),
            "Status Label": QLabel("Status: Ready")
        }
        
        # Set some initial properties
        self.toolbar_widgets["Text Input"].setPlaceholderText("Enter text...")
        self.toolbar_widgets["Text Input"].setMaximumWidth(150)
        
        # Add widgets to toolbar in desired order
        self.toolbar.addWidget(self.toolbar_widgets["Button 1"])
        self.toolbar.addWidget(self.toolbar_widgets["Button 2"])
        self.toolbar.addSeparator()
        self.toolbar.addWidget(self.toolbar_widgets["Option 1"])
        self.toolbar.addWidget(self.toolbar_widgets["Option 2"])
        self.toolbar.addSeparator()
        self.toolbar.addWidget(self.toolbar_widgets["Text Input"])
        self.toolbar.addSeparator()
        self.toolbar.addWidget(self.toolbar_widgets["Status Label"])
        
        # Connect events using partial instead of lambda
        self.toolbar_widgets["Button 1"].clicked.connect(partial(self.on_toolbar_action, "Button 1"))
        self.toolbar_widgets["Button 2"].clicked.connect(partial(self.on_toolbar_action, "Button 2"))
        self.toolbar_widgets["Option 1"].stateChanged.connect(partial(self.on_toolbar_state_changed, "Option 1"))
        self.toolbar_widgets["Option 2"].stateChanged.connect(partial(self.on_toolbar_state_changed, "Option 2"))
        self.toolbar_widgets["Text Input"].textChanged.connect(partial(self.on_toolbar_text_changed, "Text Input"))
    ...
    def add_toolbar_widget(self, name, widget, position=None):
        ...
        if name in self.toolbar_widgets:
            print(f"Warning: Toolbar widget '{name}' already exists. Use a unique name.")
            return False
    ...
    def set_toolbar_value(self, name, value):
        ...
        if hasattr(widget, 'setChecked'):  # QCheckBox
            ...
            print(f"Warning: Don't know how to set value for widget type {type(widget)}")
            return False
        return True
    ...
    def get_toolbar_value(self, name):
        ...
        if hasattr(widget, 'isChecked'):  # QCheckBox
            ...
            print(f"Warning: Don't know how to get value for widget type {type(widget)}")
            return None
    ...
    def add_widget(self, name, widget, background_color="#F0F0F0"):
        ...
        widget.setStyleSheet(f"background-color: {background_color}; border: 1px solid #CCCCCC;")
...
def main():
    ...
    success = window.custom_widget.add_toolbar_widget("Enable Debug", duplicate_checkbox)
