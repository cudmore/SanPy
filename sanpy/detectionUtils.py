...
def refineWithDerivative(y : np.ndarray, spikeTimes : list[int],
	...
	for idx,footPnt in enumerate(spikeTimes):
		...
		if len(zero_crossings)==0:
			foundFoot = False
			if verbose:
				logger.error(f'  no foot for peak {idx} at pnt {footPnt} ... did not find zero crossings')
		else:
			foundFoot = True
...
def refineWithPercentOfMax(y, spikeTimes, dvdt_percentOfMax, firstDeriv=None, verbose=False):
	...
	for idx,spikeTime in enumerate(spikeTimes):
		...
		try:
			...
			if len(tmpWhere) > 0:
				...
				eDict = getErrorDict(idx, spikeTime, errorType, errStr) # spikeTime is in pnts
				if verbose:
					logger.error(eDict)
			...
			print('   FIX ERROR: refineWithPercentOfMax() looking for dvdt_percentOfMax')
...
def keepPeaksAbove(y, spikeTimes, onlyPeaksAbove_mV : float, verbose=False):
	...
	if onlyPeaksAbove_mV is None:
		return spikeTimes
	goodSpikeTimes = [spikeTime for spikeTime in spikeTimes if y[spikeTime] > onlyPeaksAbove_mV]
	if verbose:
...
def keepPeaksBelow(y, spikeTimes, onlyPeaksBelow_mV : float, verbose=False):
	...
	if onlyPeaksBelow_mV is None:
		return spikeTimes
	goodSpikeTimes = [spikeTime for spikeTime in spikeTimes if y[spikeTime] < onlyPeaksBelow_mV]
	if verbose:
		logger.info('reduce peaks from {len(spikeTimes)} to len(goodSpikeTimes})')
	return goodSpikeTimes
...
def getHalfWidth(y, spikePnts, spikePeakPnts,
				hwWindowPnts : int,
				dataPointsPerMs,
	...
	for idx, spikePnt in enumerate(spikePnts):
		...
		hwList[idx], hwErrorList[idx] = getHalfWidth_(y, spikePnt, peakPnt, 
							idx,
							hwWindowPnts,
							dataPointsPerMs,
							halfHeights=halfHeights, verbose=verbose)
...
def getHalfWidth_(vm, thresholdPnt, peakPnt,
	...
	for j, halfHeight in enumerate(halfHeights):
		...
		try:
			...
			errorStr = (f'Half width {halfHeight} error in "{currentErrorType}" '
					f"with hwWindowPnts:{hwWindowPnts} "
					f'searching for Vm:{round(thisVm,2)} from peak sec {round(peakSec,2)}'
					)

			eDict = getErrorDict(peakIdx, thresholdPnt, errorType, errorStr) # spikeTime is in pnts
			errorList.append(eDict)
			if verbose:
				logger.error(f'peakIdx:{peakIdx} spikeSecond:{spikeSecond} halfHeight:{halfHeight} eDict:{eDict}')
...
def getMaximumDepolaizingPotential(y, spikeTimes : List[int],
				mdp_pre_pnts : int, avgWindow_pnts : int,
				verbose=False):
	...
	for idx, spikePnt in enumerate(spikeTimes):
		...
		try:
			...
			eDict = getErrorDict(idx, spikeTimes[idx], errorType, errorStr) # spikeTime is in pnts
...
def getEarlyDiastolicDurationRate(y, spikeTimes : List[int],
							preMinPnts : List[int],
	...
	for idx, spikeTime in enumerate(spikeTimes):
		...
		with warnings.catch_warnings():
			...
			try:
				...
				logger.error(f' !!!!!!!! UNKNOWN EXCEPTION DURING EDD LINEAR FIT for spike {idx}')
				retSpikeDict[idx]['earlyDiastolicDurationRate'] = defaultVal
				errorType = 'Fit EDD'
...
def peakDetect(sweepY, detectionParams : sanpy.bDetection, dataPointsPerMs : int):
	...
	if SavitzkyGolay_pnts > 0:
		sweepY = getSavitzkyGolay_Filter(sweepY, SavitzkyGolay_pnts, SavitzkyGolay_poly, verbose)
	...
	if detectionType == 'dvdt':
		...
		spikePnts = getThreshold_firstDerivative(sweepY, dvdtThreshold, firstDerivative=firstDerivative)
	...
	spikePnts, errorList = refineWithPercentOfMax(sweepY, spikePnts, dvdt_percentOfMax,
								firstDeriv=firstDerivative, verbose=verbose)
	...
	# half-width
	halfWidthWindow_pnts = detectionParams.getMsValueAsPnt('halfWidthWindow_ms', dataPointsPerMs)
	halfHeights = detectionParams['halfHeights']
	
	hwList, hwErrorList = getHalfWidth(sweepY, spikePnts, spikePeakPnts, 
							halfWidthWindow_pnts, dataPointsPerMs,
							halfHeights=halfHeights, verbose=verbose)
	...
	mdpList, mdpErrorList = getMaximumDepolaizingPotential(sweepY, spikePnts,
				mdp_pre_pnts=mdp_pre_pnts, avgWindow_pnts=avgWindow_pnts,
				verbose=verbose)

	lowEddRate_warning = detectionParams['lowEddRate_warning']
	eddRateList, eddRateErrorList = getEarlyDiastolicDurationRate(sweepY, spikePnts, mdpList,
